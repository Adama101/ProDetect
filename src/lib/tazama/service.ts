...
export class TazamaService {
  ...
  ): Promise<TazamaRuleEvaluationResult | null> {
    try {
      ...
      const result = await tazamaClient.evaluateTransaction(
        tazamaTransaction,
        tazamaCustomer
      );
  ...
  async batchProcessTransactions(
    ...
    try {
      ...
      validCustomers.forEach((customer) => {
        tazamaCustomers[customer.id] =
          TazamaMapper.mapCustomerToTazama(customer);
      });

      // Batch evaluate transactions
      const results = await tazamaClient.batchEvaluateTransactions(
        tazamaTransactions,
        tazamaCustomers
      );
      ...
      console.error(
        "Error batch processing transactions through Tazama:",
        error
      );
      return {};
    }
   ...
   */
  private async processEvaluationResult(
    result: TazamaRuleEvaluationResult,
    transactionId: string
  ): Promise<void> {
    try {
      // Update transaction risk score
      if (result.risk_score !== undefined) {
        await postgresService.updateTransactionRiskScore(
          transactionId,
          result.risk_score
        );
      }
